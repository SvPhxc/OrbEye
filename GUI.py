...
class TrackerWindow(QtWidgets.QMainWindow):
    def __init__(self, shared_data):
        super().__init__()
        # ... (initial setup, view, plots are unchanged) ...
        ...
        # ... (status label setup is unchanged) ...
        ...
        ekf_box = QtWidgets.QGroupBox("EKF / Tracker Control")
        ekf_layout = QVBoxLayout()

        # --- NEW: Checkbox to switch between trackers ---
        self.chk_use_ekf = QCheckBox("Use EKF (Predictive)")
        self.chk_use_ekf.setChecked(self.shared_data["use_ekf_tracker"].value)
        self.chk_use_ekf.toggled.connect(self.on_tracker_type_toggled)
        ekf_layout.addWidget(self.chk_use_ekf)
        # ------------------------------------------------
        ...
        self.btn_stop_ekf = QPushButton("Stop Tracking")
        self.btn_stop_ekf.clicked.connect(self.stop_tracking)
        ekf_layout.addWidget(self.btn_stop_ekf)
        ekf_box.setLayout(ekf_layout)
        controls_layout.addWidget(ekf_box)

        # ... (LCDs and shutdown button are unchanged) ...
    ...
    # --- NEW HANDLER for the tracker switch ---
    def on_tracker_type_toggled(self, checked):
        self.shared_data["use_ekf_tracker"].value = checked
        print(f"[GUI] Tracker type set to: {'EKF' if checked else 'Simple'}")

    # --- MODIFIED to be a generic stop button ---
    def stop_tracking(self):
        self.shared_data["lidar_track_mode_active"].value = False
        print("[GUI] Manual tracking stop requested.")
    ...
    def create_lcd(self):
        lcd = QtWidgets.QLCDNumber(); lcd.setSegmentStyle(QtWidgets.QLCDNumber.Flat); lcd.setDigitCount(6); return lcd

    def create_separator(self):
        line = QtWidgets.QFrame(); line.setFrameShape(QtWidgets.QFrame.HLine); line.setFrameShadow(
            QtWidgets.QFrame.Sunken); return line
    ...
    def on_go_clicked(self):
        try:
            self.shared_data["target_azimuth"].value = float(self.az_input.text()); self.shared_data[
                "target_elevation"].value = float(self.el_input.text()); self.shared_data["go_to_target"].value = True
        except ValueError:
    ...
    def update_ui(self):
        ...
        # Update 3D view (unchanged)
        try:
            az, el = self.shared_data['estimated_azimuth'].value, self.shared_data['estimated_elevation'].value if \
            self.shared_data['ekf_initialized'].value else (self.shared_data['stepper_degrees'].value,
                                                            self.shared_data['servo_degrees'].value)
            dist_m = 10.0  # Dummy distance for visualization
            az_rad, el_rad = np.radians(az), np.radians(el);
            x, y, z = dist_m * np.cos(el_rad) * np.cos(az_rad), dist_m * np.cos(el_rad) * np.sin(
                az_rad), dist_m * np.sin(el_rad)
            self.laser.setData(pos=np.vstack((np.zeros(3), np.array([x, y, z]))));
            self.satellite.setData(pos=np.array([x, y, z]).reshape(1, 3))
        except Exception:
            pass

    def Pshutdown(self):
        self.shared_data["shutdown"].value = True; self.timer_ui.stop(); QtCore.QTimer.singleShot(250,
                                                                                                  QtWidgets.QApplication.instance().quit)

    def closeEvent(self, event):
        self.Pshutdown(); super().closeEvent(event)
