# hardware_controller.py (Corrected for State Machine Logic)
    ...
    def run(self):
        try:
            ...
            while not self.shared_data["shutdown"].value:
                ...
                # --- CODE REPAIRED HERE ---
                # The `target_reached` check has been moved inside each state's logic block.
                # This ensures the check is always performed against the *current* target.
                ...
                if current_state == "HF_TRACKING":
                    ...
                    if target_reached:
                        # Once at the point, try to get a measurement
                        try:
                            ...
                            elif current_state == "SEARCHING":  # (Logic for SEARCHING remains here)
                                pass
                        except queue.Empty:
                            # If there's no data, we still log a point, but maybe with a placeholder
                            # For now, we'll just skip and move to the next point
                            pass

                        # After attempting measurement, move to the next pattern point
                        if not self._update_scan_pattern():
                            print(f"[HWCtrl] {current_state} finished.")
                            if current_state == "BACKGROUND_SCAN":
                                self.shared_data["background_scan_active"].value = False
                    else:
                        # If target is NOT reached, command the motors to move
                        pan_vel, tilt_vel = self.pan_pid.update(self.internal_pan_pos), self.tilt_pid.update(
                            self.internal_tilt_pos)
