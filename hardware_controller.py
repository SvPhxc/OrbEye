...
class HardwareController:
    def __init__(self, shared_data):
        ...
        self.pi=None; self.ser=None; self.shutdown_event=threading.Event()
    ...
    def _execute_motor_commands(self, pan_vel_dps, tilt_vel_dps, dt):
        if abs(pan_vel_dps)>0.1:
            ...
            self.internal_pan_pos=(self.internal_pan_pos+pan_vel_dps*dt+360)%360
        ...
        try:
            ...
            while not self.shared_data["shutdown"].value:
                ...
                if next_state != current_state:
                    print(f"[HWCtrl] State: {current_state} -> {next_state}")
                    self.pan_pid.reset(); self.tilt_pid.reset(); current_state=next_state

                pan_vel, tilt_vel = 0, 0
                if current_state == "GOTO_POSITION":
                    # In tracking mode, the target is the EKF prediction. Otherwise, it's the manual target.
                    target_az = self.shared_data["predicted_azimuth"].value if is_tracking else self.shared_data["target_azimuth"].value
                    target_el = self.shared_data["predicted_elevation"].value if is_tracking else self.shared_data["target_elevation"].value
                    ...
                    if abs(pan_err) < TARGET_REACHED_THRESHOLD_DEG and abs(tilt_err) < TARGET_REACHED_THRESHOLD_DEG:
                        ...
                        if not is_tracking: self.shared_data["go_to_target"].value = False # Stop manual moves on arrival
                    else:
