...
'''if __name__ == '__main__':
    with Manager() as manager:
        ...
        motor_process = Process(target=run_motor_control, args=(shared_data, movement_queue))
        motor_process.start()

        try:
            # --- Example of how to control the motor process ---
            print("[Main] Waiting for motor control to initialize...")
            sleep(3) 

            # Trigger a scan
            print("[Main] Triggering concentric scan...")
            shared_data['scan_trigger'] = True

            # Wait until scan is done (scan_trigger will be set to False)
            while shared_data['scan_trigger']:
                sleep(0.5)
            
            print("[Main] Scan complete. Waiting 2 seconds.")
            sleep(2)
            
            # Command a manual move
            print("[Main] Commanding manual move: pan left")
            shared_data['pan_left'] = True
            sleep(2) # Give it time to execute
