...
from tracker_logic import run_tracker_logic  # <-- NEW PROCESS
...
if __name__ == "__main__":
    ...
    shared_data = {
        ...
        # --- Active Tracker (Overall System) ---
        "lidar_track_mode_active": Value('b', False),
        "satellite_detected": Value('b', False),

        # --- NEW: Grid Scan Coordination ---
        "grid_scan_request": Value('b', False),
        "grid_scan_target_az": Value('d', 0.0),
        "grid_scan_target_el": Value('d', 0.0),
        "grid_scan_results": Array('d', [0.0] * 9), # Stores 9 distance measurements
        "grid_scan_complete": Value('b', False),

        # --- EKF State (Managed by TrackerLogic) ---
    ...
    processes = {
        ...
        "TrackerLogic": Process(target=run_tracker_logic, args=(shared_data,)), # <-- NEW
    ...
    try:
        ...
        join_or_escalate(processes["TrackerLogic"], "TrackerLogic") # <-- NEW
