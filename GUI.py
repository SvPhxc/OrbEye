...
from PyQt5.QtWidgets import QVBoxLayout, QPushButton, QCheckBox  # Explicitly import for clarity
...
class TrackerWindow(QtWidgets.QMainWindow):
    ...
    def update_readouts(self):
        ...
        if self.shared_data["lidar_track_mode_active"].value:
            self.status_label.setText("Status: HF TRACKING"); self.status_label.setStyleSheet("color: #D22B2B;")
        elif self.shared_data["background_scan_active"].value:
            self.status_label.setText("Status: SCANNING"); self.status_label.setStyleSheet(
                "color: #FFBF00;"); self.btn_scan.setChecked(True)
        elif self.shared_data["search_mode_active"].value:
            self.status_label.setText("Status: SEARCHING"); self.status_label.setStyleSheet(
                "color: #FFBF00;"); self.btn_search.setChecked(True)
        elif self.shared_data["go_to_target"].value:
            if self.shared_data["target_reached"].value:
                self.status_label.setText("Status: HOLDING POSITION"); self.status_label.setStyleSheet(
                    "color: #228B22;")
            else:
                self.status_label.setText("Status: MOVING"); self.status_label.setStyleSheet("color: #33F;")
        else:
