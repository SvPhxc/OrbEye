...
class TrackerWindow(QtWidgets.QMainWindow):
    def __init__(self, shared_data):
        ...
        ### NEW ### - Button to activate the auto-tracking mode
        self.btn_auto_track = QPushButton("Start Auto-Acquire & Track")
        ...
        self.chk_hf_tracking = QCheckBox("Enable High-Frequency Tracking (EKF)")
        ...
        # ... (rest of the GUI code is unchanged)
    ...
    ### NEW ### - Handler for the new auto-track button
    def on_auto_track_toggled(self, checked):
        """Toggles the automatic target acquisition and tracking mode."""
        self.shared_data["auto_track_active"].value = checked
        print(f"[GUI] Auto-Track mode set to: {checked}")
    ...
    def create_separator(self):
        ...
        return line
    def on_acquire_clicked(self): self.shared_data["acquire_points"].value = True
    def on_scan_toggled(self, checked): self.shared_data["background_scan_active"].value = checked
    def on_hf_track_toggled(self, checked): self.shared_data["lidar_track_mode_active"].value = checked
    ...
    def on_go_clicked(self):
        ...
        except ValueError: print("[GUI] Invalid go-to coordinates.")
    ...
    def update_ui(self):
        ...
        # ### NEW ### - Updated status label to include the new mode
        if self.shared_data["auto_track_active"].value:
             self.status_label.setText("Status: AUTO-TRACK"), self.status_label.setStyleSheet("color: #9400D3;") # Violet
            ...
            self.status_label.setText("Status: TRACKING (EKF)"), self.status_label.setStyleSheet("color: #D22B2B;")
        ...
        try:
            az, el, dist_cm = self.shared_data['stepper_degrees'].value, self.shared_data['servo_degrees'].value, self.shared_data['lidar_data'][0]
            length_m = dist_cm / 100.0 if 10.0 <= dist_cm <= 16000.0 else 15.0
            az_rad, el_rad = np.radians(az), np.radians(el)
            x, y, z = length_m * np.cos(el_rad) * np.cos(az_rad), length_m * np.cos(el_rad) * np.sin(az_rad), length_m * np.sin(el_rad)
            tip = np.array([x, y, z])
