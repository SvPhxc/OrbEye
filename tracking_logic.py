...
class HandTracker:
    ...
    def __init__(self, scan_radius=8, scan_points=8, time_per_waypoint=0.04, timeout=1.0, coast_timeout=1.5,
    ...
    def update(self, current_az, current_el, measurement, shared_data):
        ...
        # --- NEW: COASTING STATE LOGIC ---
            ...
            predicted_delta_az = 4.2*self.smoothed_velocity['az'] * (dt / (self.scan_points * self.time_per_waypoint))
            predicted_delta_el = 2*self.smoothed_velocity['el'] * (dt / (self.scan_points * self.time_per_waypoint))
    ...
    def worker():
        try:
            ...
            run_orbit_patrol_from_query(
                ...
                spiral_radius_deg = 3,
                spiral_rotations = 4  # <-- NEW (optional; 2 rotations)
...
def run_tracker_process(shared_data):
    ...
    while not shared_data["shutdown"].value:
        try:
            ...
            measurement_valid = (80.0 <= dist <= 240.0)
            ...
            if shared_data["orbit_patrol_start"].value and not shared_data["orbit_patrol_active"].value:
                if not shared_data["background_scan_active"].value:
                    shared_data["orbit_patrol_start"].value = False
                    _start_orbit_patrol(shared_data)
                else:
                    print("[Patrol] Refusing to start during BACKGROUND_SCAN.")
